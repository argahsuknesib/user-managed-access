...
export class OwnerUtil {
  ...
  public async findIssuer(webid: string): Promise<string | undefined> {
    if (!this.umaServerURL) {
      this.logger.warn(`No UMA Authorization Server variable set. Falling back on http://n063-02b.wall2.ilabt.iminds.be:4000/`)
      return 'http://n063-02b.wall2.ilabt.iminds.be:4000/uma';
    }
